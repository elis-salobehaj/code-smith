...
It also reflects Code Smith's shipped `.codesmith.yaml` repo-review-config foundation and current consumers: repo-root discovery, YAML parsing, strict validation, safe fallback defaults, pre-agent diff filtering, severity-aware verdict policy, and prompt injection for repo-owned review guidance. Linter-profile consumers and output shaping are still not scored as delivered yet.
...
- **Repo config is still incomplete, but the core review loop is now wired.** `.codesmith.yaml` is now loaded, validated, documented, and enforced for diff exclusion, skip rules, severity-aware verdict policy, and prompt injection. Feature toggles, linter-profile consumers, and output shaping are still planned.
...
| Customization & extensibility | Code Smith now combines unrestricted source access with a shipped repo-owned `.codesmith.yaml` configuration contract plus active runtime consumers for exclusions, severity policy, and prompt injection. That closes more of the ergonomics gap with GitLab Duo and CodeRabbit, which both already expose repo-level YAML customization. Richer output consumers are still planned. |
