...
It also reflects Code Smith's shipped `.codesmith.yaml` repo-review-config foundation and current pipeline consumers: repo-root discovery, YAML parsing, strict validation, safe fallback defaults, pre-agent diff filtering, and severity-aware verdict policy. Prompt injection, linter-profile consumers, and output shaping are still not scored as delivered yet.
...
- **Repo config is still incomplete, but no longer foundation-only.** `.codesmith.yaml` is now loaded, validated, documented, and enforced for diff exclusion, skip rules, and severity-aware verdict policy. Prompt injection, feature toggles, linter-profile consumers, and output shaping are still planned.
...
| Customization & extensibility | Code Smith now combines unrestricted source access with a shipped repo-owned `.codesmith.yaml` configuration contract plus active pipeline consumers for exclusions and severity policy. That closes more of the ergonomics gap with GitLab Duo and CodeRabbit, which both already expose repo-level YAML customization. Prompt injection and richer output consumers are still planned. |
